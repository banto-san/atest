...
<aside class="sidebar">
    ...
    <a class="nav<?= $a('dashboard') ?>" href="index.php"><?= icon('key') ?> API一覧</a>
    <a class="nav<?= $a('cost') ?>" href="<?= h(app_url('cost')) ?>"><?= icon('dashboard') ?> コスト</a>
    <?php if (can_edit()): ?><a class="nav<?= $a('manage') ?>" href="<?= h(app_url('manage')) ?>"><?= icon('gear') ?> コスト取得キー</a><?php endif; ?>
    <?php if (can_edit()): ?><a class="nav<?= $a('accounts') ?>" href="<?= h(app_url('accounts')) ?>"><?= icon('lock') ?> アカウント管理</a><?php endif; ?>
    <a class="nav<?= $a('myaccounts') ?>" href="<?= h(app_url('myaccounts')) ?>"><?= icon('shield') ?> 個人アカウント</a>
