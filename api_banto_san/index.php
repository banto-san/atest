...
// 手動追加API（使用箇所ゼロ・箱なし）も必ずカード表示するため、apis から直接プロダクトを登録。
// あわせて API 自体に入力した月額をプロダクト別に集計（箱コストとは別枠で加算する）。
$apiMoneyByProduct = [];      // productName => [currency => sum]
foreach ($apis as $a) {
    $pn = (string) $a['name'];
    if ($pn === '') { continue; }
    if (!isset($providerOf[$pn])) { $providerOf[$pn] = $a['provider']; }   // 使用箇所が無くてもカード化
    if ($a['monthly_cost'] !== null) {
        $cur = $a['currency'] ?: 'JPY';
        $apiMoneyByProduct[$pn][$cur] = ($apiMoneyByProduct[$pn][$cur] ?? 0) + (float) $a['monthly_cost'];
    }
}
// プロダクト一覧 = 使用箇所のプロダクト ∪ 箱のプロダクト ∪ 手動追加API
$tree = [];
...
// プロダクトのコスト合計（配下の箱コスト＋API月額の和）と並べ替え
$productTotalNum = [];
foreach (array_keys($tree) as $name) {
    ...
    foreach (($apiMoneyByProduct[$name] ?? []) as $v) { $sum += (float) $v; }
...
foreach ($apiMoneyByProduct as $byCur) {
    foreach ($byCur as $cur => $v) { $subtotals[$cur] = ($subtotals[$cur] ?? 0) + (float) $v; }
}
ksort($subtotals);
...
foreach ($names as $nm) {
    ...
    foreach (($apiMoneyByProduct[$nm] ?? []) as $cur => $v) {
        $prodCostByCur[$cur][$nm] = ($prodCostByCur[$cur][$nm] ?? 0) + (float) $v;
    }
}
...
if ($route === 'product'):
    ...
    // コスト（通貨別）＝箱コスト＋このプロダクトのAPI月額
    $pcost = [];
    foreach ($pboxes as $b) {
        if ($b['monthly_cost'] !== null) { $c = $b['currency'] ?: 'USD'; $pcost[$c] = ($pcost[$c] ?? 0) + (float) $b['monthly_cost']; }
    }
    foreach (($apiMoneyByProduct[$pname] ?? []) as $c => $v) { $pcost[$c] = ($pcost[$c] ?? 0) + (float) $v; }
    ksort($pcost);
...
<main class="main">
    ...
    <div class="card-grid">
        <?php foreach ($names as $gname):
            ...
            foreach (($apiMoneyByProduct[$gname] ?? []) as $cur => $v) { $cmoney[$cur] = ($cmoney[$cur] ?? 0) + (float) $v; }
            ksort($cmoney);
