...
PROXY      = ""            # 社内プロキシがある場合のみ "http://proxy.xxx.co.jp:8080" 形式で設定（空なら自動/直結）
...
# ----------------------------------------------------------------------
def make_session():
    """requests セッションを作成（プロキシ・証明書検証・リトライを設定）。"""
    s = requests.Session()
    ...
    if PROXY:
        ...
        s.trust_env = False        # 明示プロキシを優先（Windowsのプロキシ設定より優先）
...
def import_csv(path):
    ...
    s = make_session()
    log(f"接続設定: プロキシ={PROXY or (urllib.request.getproxies() or 'なし(直結)')} / 証明書検証={VERIFY_SSL}", 1)
