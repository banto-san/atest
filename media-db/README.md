...
| `layout_top.php` | 共通レイアウト上部（サイドメニュー＋ヘッダー）。Vue は `#app`（メイン領域）にマウント |
| `layout_bottom.php` | 共通レイアウト下部。初期データ・CSRFを埋め込み、`app-core.js`→ページ専用JSの順で読込 |
| `app-core.js` | 全ページ共通：単一データストア / 自動保存 / 媒体集計・ランキング / CSV出力 |
| `page-*.js` | 各ページ専用の Vue アプリ（`page-dashboard.js` など） |
| `data.json` | 実データ（初回アクセス時に自動生成・**gitignore済み**。サーバー上の実データは上書きしない） |

> すべてのファイルは `media-db/` 直下に置く**フラット構成**（サブフォルダなし）。
> アップロード時はこの1フォルダに全ファイルを入れるだけでよい。
